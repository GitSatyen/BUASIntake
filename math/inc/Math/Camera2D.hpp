...
namespace Math
{
    ...
    inline AABB operator*(const glm::mat3 transform, const AABB& aabb) noexcept
    {

        const auto min = transform * glm::vec3{ aabb.min.x, aabb.min.y, 1.0f };
        const auto max = transform * glm::vec3{ aabb.max.x, aabb.max.y, 1.0f };
